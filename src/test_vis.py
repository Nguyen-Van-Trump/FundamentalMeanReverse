...
from config.settings import MARKET_DATA_DIR, TIME_COLUMN


def load_symbol(symbol: str):

    file = MARKET_DATA_DIR / f"symbol={symbol}" / "data.parquet"
...
def info(symbol: str):
    ...
    # plt.plot(df[TIME_COLUMN], df["close"], label="Close Price")
